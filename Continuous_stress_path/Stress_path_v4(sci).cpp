...
void StressOptimize() {
	//根据应力对应力小的区域进行过滤
	double v = 0.15;//过滤百分比,gong = 0.05;ls = ? ; qiao = 0.1
	double a;
	bool flag;
	element s;
...
void GcodePrint() {
	...
	double t2 = 0.04991;//层厚0.2，丝宽0.56
...
void main(int argc, char** argv) {
	...
	for (int i = 0; i < paths.size(); i++) {
		...
		for (int j = 0; j < paths[i].size(); j++) {
			for (int m = 1; m < paths[i][j].size(); m++) {
				if (paths[i][j][m - 1].x == paths[i][j][m].x || paths[i][j][m - 1].y == paths[i][j][m].y)
					dis1 += distance(paths[i][j][m - 1], paths[i][j][m]);
				else
					dis2 += distance(paths[i][j][m - 1], paths[i][j][m]);
			}
		}
	}
	printf("第一层，水平或竖直的路径长度为%f,倾斜路径的长度为%f\n", dis1, dis2);
	int count11 = 0;
		//for (int i = 0; i < point.size(); i++) {
			for (int j = 0; j < point[0].size(); j++) {
				for (int m = 0; m < point[0][j].size(); m++) {
