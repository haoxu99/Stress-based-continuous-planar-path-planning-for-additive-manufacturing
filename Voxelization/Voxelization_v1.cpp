...
const string file_1 = "tooth.obj";//读取文件
...
//*********************将最内圈的点赋给shell1***********************************
	...
	for (int i = 0; i < v_point.size(); i++) {
		for (int k = 0; k < shell1[i].size(); k++) {
			polypoint.clear();//把上层的缓存线删除
			polypoint = shell1[i][k];//把第i层的线段坐标进行缓存
...
void InitElement() {
	...
	for (int i = 0; i < node3.size()-1; i++) {
		for (int j = 0; j < node3[i].size()-1; j++) {
			for (int m = 0; m < node3[i][j].size()-1; m++) {
				n++;
				e.id = n; 
				e.n1 = node3[i][j][m].id;
