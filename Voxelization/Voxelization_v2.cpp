...
const string file_1 = "ls-90.obj";//读取文件
...
void InitVoxelization(float x,float y,float z) {//创建包围盒体素点
	voxelization v;
	double v_x, v_y, v_z, l1, l2;
	l1 = ((double)4 / (double)3) * y;
	l2 = (2 * sqrt(3) / (double)3) * x;
	v_x = Bmin_x + l2 / (double)2;
	v_y = Bmin_y + y * ((double)2 / (double)3);
	v_z = Bmin_z + z / (double)2;
	int num = 0;
	for (v_y; v_y < Bmax_y; v_y += y) {
		if (num % 2 == 0) v_x = Bmin_x + l2 / (double)2;
		else if (num % 2 == 1) v_x = Bmin_x + l2;
		for (v_x; v_x < Bmax_x; v_x += l2) {
			v.x = v_x;
			v.y = v_y;
			v.b = false;
			v_coord.push_back(v);
...
void DataOutput(float v_z) {//输出体素数据
	...
	for (int i = 0; i < v_point.size(); i++) {
		...
		for (int j = 0; j < v_point[0].size(); j++) {
			...
			//if (v_point[i][j].b == 1) {
			//	fprintf(fp1, "%f %f %f\n", v_point[i][j].x, v_point[i][j].y, z);//单元号，x,y,z的坐标，bool值
			//}
		}
	
	}
	/*for (int j = 0; j < model[0].size(); j++) {
		fprintf(fp1, "%f %f ", model[0][j].x, model[0][j].y);
	}*/
	fclose(fp);
	//fclose(fp1);
}
