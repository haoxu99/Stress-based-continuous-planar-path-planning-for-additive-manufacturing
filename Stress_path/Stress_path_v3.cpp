...
void Zigzag_0() {
	...
	for (int i = 0; i < z_count; i++) {
		for (int j = 0; j < y_count; j++) {
			for (int m = 0; m < x_count; m++) {
				if (point[i][j][m].b == 1) {
					s.id = point[i][j][m].id; s.x = point[i][j][m].x; s.y = point[i][j][m].y; s.z = point[i][j][m].z;
...
void Zigzag_90() {
	...
	for (int i = 0; i < z_count; i++) {
		for (int j = 0; j < x_count; j++) {
			for (int m = 0; m < y_count; m++) {
				if (point[i][m][j].b == 1) {
					s.id = point[i][m][j].id; s.x = point[i][m][j].x; s.y = point[i][m][j].y; s.z = point[i][m][j].z;
					paths_buffer1.push_back(s);
					n++;
				}
				else if (n != 0) {
...
//				if (m == j) m++;
//				if (m == paths[i].size()) break;
//				if (paths[i][m][0].id == paths[i][j][0].id + paths[i][j][0].id - paths[i][j][1].id && paths[i][j][0].id % x_count != 0 && paths[i][j][0].id % x_count != 1) {
...
//				if (m == j) m++;
//				if (m == paths[i].size()) break;
//				if (paths[i][m][paths[i][m].size() - 1].id == paths[i][j][paths[i][j].size()-1].id + paths[i][j][paths[i][j].size() - 1].id - paths[i][j][paths[i][j].size() - 2].id && paths[i][j][paths[i][j].size() - 1].id % x_count != 0 && paths[i][j][paths[i][j].size() - 1].id % x_count != 1) {
...
//				if (m == j) m++;
//				if (m == paths[i].size()) break;
//				if (paths[i][m][0].id == paths[i][j][paths[i][j].size() - 1].id + paths[i][j][paths[i][j].size() - 1].id - paths[i][j][paths[i][j].size() - 2].id && paths[i][j][paths[i][j].size() - 1].id % x_count != 0 && paths[i][j][paths[i][j].size() - 1].id % x_count != 1) {
//						paths[i][j].insert(paths[i][j].end(), paths[i][m].begin(), paths[i][m].end());
//						paths[i].erase(paths[i].begin() + m);
//						m = 0; j = 0;
//						break;
...
//				if (m == j) m++;
//				if (m == paths[i].size()) break;
//				if (paths[i][m][paths[i][m].size()-1].id == paths[i][j][0].id + paths[i][j][0].id - paths[i][j][1].id && paths[i][j][0].id % x_count != 0 && paths[i][j][0].id % x_count != 1) {
...
void GcodePrint() {
	...
	err = fopen_s(&fp, "zigzag_0.gcode", "a"); //若return 1 , 则将指向这个文件的文件流给
	double t1 = 0.04656;//层厚0.2，丝宽0.4
	double t2 = 0.04656;//层厚0.2，丝宽0.56
	double E = 0;
	double r;//回抽
	...
	//模型第一层
	fprintf(fp, "G0 F9000 X%f Y%f Z%f\n", paths[0][0][0].x, paths[0][0][0].y, paths[0][0][0].z + 0.1);//模型第一层与底座间隔0.22
